...
velext_1=0.6/(100*365.25*24*3600); % Give in cm/yr and with / adapt to m/s
velext_2=3.2/(100*365.25*24*3600); % Give in cm/yr and with / adapt to m/s

% End simulations after fixed amount of extension
time_end_1 = 120e3 / velext_1;
time_end_2 = time_end_1 + (320e3 / velext_2);
...
% 4 = Intrusion granite
MRHO(4,1)=2600;             % standard density, kg/m^3
...
MFLOW(4,2)=1.25e-9;          % AD, 1/s/MPa^n
MFLOW(4,3)=3.2;             % n
MFLOW(4,4)=123*1e3;         % Ea, J/mol
...
MFLOW(8,2)=1.26e-3;          % AD, 1/s/MPa^n
MFLOW(8,3)=2.4;             % n
MFLOW(8,4)=219*1e3;         % Ea, J/mol
...
MFLOW(9,2)=1.26e-3;          % AD, 1/s/MPa^n
MFLOW(9,3)=2.4;             % n
MFLOW(9,4)=219*1e3;         % Ea, J/mol
...
MRHO(10,1)=2800;             % standard density, kg/m^3
...
MFLOW(10,2)=1.26e-3;          % AD, 1/s/MPa^n
MFLOW(10,3)=2.4;             % n
MFLOW(10,4)=219*1e3;         % Ea, J/mol
...
%      but done for layering to visualize deformation)
MRHO(11,1)=2800;             % standard density, kg/m^3
...
MFLOW(11,2)=1.26e-3;          % AD, 1/s/MPa^n
MFLOW(11,3)=2.4;             % n
MFLOW(11,4)=219*1e3;         % Ea, J/mol
...
save_path = './Figures_Mhina_acceleration_complete/';
if (exist(save_path, 'dir') == 0)
    mkdir(save_path);
end
output_path = './Output_acceleration_complete/';
...
for j=1:1:xnum+1
    ...
    bbottom(j,3)=-velext_1/xsize*ysize;
    bbottom(j,4)=0;
end
...
for xm = 1:1:mxnum
    for ym = 1:1:mynum
        ...
        %if (MY(mm1)>27000 && MY(mm1)<=32000 && MX(mm1)<=150000 && MX(mm1)>250000)
        if (MY(mm1)>27000 && MY(mm1)<=32000)
           if (MY(mm1)>30000 && MY(mm1)<=32000 && MX(mm1)>150000 && MX(mm1)<=250000)
               MI(mm1) = 4;
           else
            MI(mm1) = 10;
           end
      ...
      %     MI(mm1)=10;
       % end
        if(MY(mm1)>32000 && MY(mm1)<=37000)
            MI(mm1)=11;
        end
        if(MY(mm1)>37000 && MY(mm1)<=42000)
            MI(mm1)=10;
        end
        if(MY(mm1)>42000 && MY(mm1)<=47000)
            MI(mm1)=11;
        end
        % 5 = Lithospheric mantle (layering is to see deformation)
        if(MY(mm1)>47000 && MY(mm1)<=52000)
            MI(mm1)=5;
        end
%         if(MY(mm1)>52000 && MY(mm1)<=57000)
%             MI(mm1)=5;
%         end
      %  if(MY(mm1)>67000 && MY(mm1)<=72000)
      %      MI(mm1)=5;
      %  end
       % if(MY(mm1)>77000 && MY(mm1)<=82000)
        %    MI(mm1)=5;
       % end
       % if(MY(mm1)>87000 && MY(mm1)<=92000)
        %    MI(mm1)=5;
       % end
       % if(MY(mm1)>97000 && MY(mm1)<=102000)
        %    MI(mm1)=5;
        %end
        ...
        yast=57000;                     % Lithosphere-astenophere boundary
...
for ntimestep=1:1:stepmax
    ...
    for mm1 = 1:1:marknum
        ...
        if (MX(mm1)>=gridx(1) && MX(mm1)<=gridx(xnum) && MY(mm1)>=gridy(1) && MY(mm1)<=gridy(ynum))
            ...
            if(MFLOW(MI(mm1),1)==0)
                ...
                eii0= ad_plawexp* (sii0*1e-3)^(MFLOW(MI(mm1),3));
    ...
    savestepfig = 20; % set to 1 if you want to debug with high-resolution figure output
...
%         print('-dtiff','-r200',filename_fig);
    ...
    ysize=ysize-velext_1/xsize*ysize*timestep; % Vertical size
    xsize=xsize+velext_1*timestep; % Horizontal size
...
for j=1:1:xnum+1
    ...
    bbottom(j,2)=1;
    % vy(ynum,j)=bbottom(j,3)+vy(ynum-1,j)*bbottom(j,4)
    bbottom(j,3)=-velext_2/xsize*ysize;
    bbottom(j,4)=0;
end
...
% Second Time cycle (10-0 Ma)
disp 'Start timestepping2';
for ntimestep=1:1:stepmax
    ...
    if(fix(ntimestep/savestepfig)*savestepfig==ntimestep)
        ...
        % --- Plot stress and strain ------------------------------------
         ...
         if ntimestep<10
             filename_fig    =  [save_path,'stress_strain2','00', num2str(ntimestep)];
         elseif ntimestep<100 &&  ntimestep>=10
             filename_fig    =  [save_path,'stress_strain2','0', num2str(ntimestep)];
         else
             filename_fig    =  [save_path,'stress_strain2', num2str(ntimestep)];
         end
...
%         print('-dtiff','-r200',filename_fig);
        ...
        if ntimestep<10
            filename_fig    =  [save_path,'comp_vis2','00', num2str(ntimestep)];
        elseif ntimestep<100 &&  ntimestep>=10
            filename_fig    =  [save_path,'comp_vis2','0', num2str(ntimestep)];
        else
            filename_fig    =  [save_path,'comp_vis2', num2str(ntimestep)];
        end
    ...
    % Defining new gridline positions for irregular basic grid
    % Computing model size change
    ysize=ysize-velext_2/xsize*ysize*timestep; % Vertical size
    xsize=xsize+velext_2*timestep; % Horizontal size
    % Define position of first nodal point
    gridx(1)=gridx(81)-xsize/2;
    % Define factor of grid spacing increase to the right
    % of high resolution area
